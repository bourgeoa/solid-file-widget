/**
 * node-solid-server connect widget
 * @constructor
 *
 * @param {object}  node-solid-server      - nodeSolidServer instance
 ...
 */
let Widget = function(nodeSolidServer, options={}) {
  this.rs = nodeSolidServer;
  ...
  // true if we have nodeSolidServer connection's info
  this.active = false;

  // nodeSolidServer is connected!
  this.online = false;
...
//        let connectedUser = this.rs.remote.userAddress;
		  this.rs.checkSession().then( session => {
			let connectedUser = this.session.webId;
//			this.ssState = "connected";
			}, err => {console.log(err); let connectedUser = "";
//			this.ssState = "disconnected";
			});
        this.rsConnectedUser.innerHTML = connectedUser;
//        this.setBackendClass(this.rs.backend);  // si pas de paramètre
...
*/
      case "error":
        this.setBackendClass(this.rs.backend);     // a voir
  ...
  setState (state) {
    if (state) {
      ...
      this.init = "initial";
console.log("1  ${this.rsWidget.className}  2  ${currentStateClass} 3  ${state} 4  ${this.state}  5  ${this.init}");
      this.rsWidget.classList.add("rs-state-${state || this.state || this.init}");
  ...
  createHtmlTemplate () {
    ...
    style.innerHTML = require("raw!./assets/styles.css"); // raw!
    element.id = "remotestorage-widget";
    element.innerHTML = require("html!./assets/widget.html"); // html!
...
*/
   ...
   * @private
   */
  setupHandlers () {   // check use of rs.on
...
//					let connectedUser = this.session.webId;
  ...
  setEventListeners () {
    ...
    this.rsSignInForm.addEventListener("submit", (e) => {
      e.preventDefault();
      let idp = document.querySelector("input[name=rs-user-address]").value;
      this.rsConnectButton.disabled = true;
      //  this.rs.connect(userAddress);
			this.fileClient.login(idp).then( webId => {
				console.log( "Logged in as ${webId}.");
			}, err => {console.log(err); eventHandler("disconnected")});
		});
  },
